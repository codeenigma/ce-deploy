...
usage(){
  echo 'deploy.sh [OPTIONS] --repo <git repo to deploy> --branch <branch to deploy> --playbook <path to playbook> --build-number < incremental build number> --build-id <custom identifier>'
  echo 'deploy.sh [OPTIONS] --workspace <local path> --playbook <path to playbook> --build-number < incremental build number> --build-id <custom identifier>'
  ...
  echo '--gitlab-project-name: If defined, the value set here will be used to set the project name in the deploy config in the repo'
}
